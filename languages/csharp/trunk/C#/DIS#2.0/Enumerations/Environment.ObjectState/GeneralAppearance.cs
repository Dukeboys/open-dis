...
namespace OpenDis.Enumerations.Environment.ObjectState
{
    ...
    public struct GeneralAppearance
    {
        ...
        public static GeneralAppearance FromUInt16(ushort value)
        {
            ...
            ps.PercentComplete = (byte)a0Value;

            uint a1Mask = 0x0300;
            byte a1Shift = 8;
            uint a1Value = value & a1Mask >> a1Shift;
            ps.Damage = (GeneralAppearance.DamageValue)a1Value;

            uint a2Mask = 0x0400;
            byte a2Shift = 10;
            uint a2Value = value & a2Mask >> a2Shift;
            ps.Predistributed = (GeneralAppearance.PredistributedValue)a2Value;

            uint a3Mask = 0x0800;
            byte a3Shift = 11;
            uint a3Value = value & a3Mask >> a3Shift;
            ps.State = (GeneralAppearance.StateValue)a3Value;

            uint a4Mask = 0x1000;
            byte a4Shift = 12;
            uint a4Value = value & a4Mask >> a4Shift;
            ps.Smoking = (GeneralAppearance.SmokingValue)a4Value;

            uint a5Mask = 0x2000;
            byte a5Shift = 13;
            uint a5Value = value & a5Mask >> a5Shift;
            ps.Flaming = (GeneralAppearance.FlamingValue)a5Value;
