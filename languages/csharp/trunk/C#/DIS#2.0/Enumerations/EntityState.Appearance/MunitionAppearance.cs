...
namespace OpenDis.Enumerations.EntityState.Appearance
{
    ...
    public struct MunitionAppearance
    {
        ...
        public static MunitionAppearance FromUInt32(uint value)
        {
            ...
            ps.Damage = (MunitionAppearance.DamageValue)a1Value;

            uint a2Mask = 0x0060;
            byte a2Shift = 5;
            uint a2Value = value & a2Mask >> a2Shift;
            ps.Smoke = (MunitionAppearance.SmokeValue)a2Value;

            uint a3Mask = 0x0180;
            byte a3Shift = 7;
            uint a3Value = value & a3Mask >> a3Shift;
            ps.TrailingEffects = (MunitionAppearance.TrailingEffectsValue)a3Value;

            uint a5Mask = 0x8000;
            byte a5Shift = 15;
            uint a5Value = value & a5Mask >> a5Shift;
            ps.Flaming = (MunitionAppearance.FlamingValue)a5Value;

            uint a6Mask = 0x10000;
            byte a6Shift = 16;
            uint a6Value = value & a6Mask >> a6Shift;
            ps.LaunchFlash = (MunitionAppearance.LaunchFlashValue)a6Value;

            uint a8Mask = 0x200000;
            byte a8Shift = 21;
            uint a8Value = value & a8Mask >> a8Shift;
            ps.FrozenStatus = (MunitionAppearance.FrozenStatusValue)a8Value;

            uint a9Mask = 0x400000;
            byte a9Shift = 22;
            uint a9Value = value & a9Mask >> a9Shift;
            ps.PowerPlantStatus = (MunitionAppearance.PowerPlantStatusValue)a9Value;

            uint a10Mask = 0x800000;
            byte a10Shift = 23;
            uint a10Value = value & a10Mask >> a10Shift;
            ps.State = (MunitionAppearance.StateValue)a10Value;

            uint a12Mask = 0x80000000;
            byte a12Shift = 31;
            uint a12Value = value & a12Mask >> a12Shift;
            ps.MaskedCloaked = (MunitionAppearance.MaskedCloakedValue)a12Value;
