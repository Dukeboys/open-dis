...
namespace OpenDis.Enumerations.EntityState.Appearance
{
    ...
    public struct SensorEmitterAppearance
    {
        ...
        public static SensorEmitterAppearance FromUInt32(uint value)
        {
            ...
            ps.PaintScheme = (SensorEmitterAppearance.PaintSchemeValue)a0Value;

            uint a1Mask = 0x0002;
            byte a1Shift = 1;
            uint a1Value = value & a1Mask >> a1Shift;
            ps.Mobility = (SensorEmitterAppearance.MobilityValue)a1Value;

            uint a2Mask = 0x0004;
            byte a2Shift = 2;
            uint a2Value = value & a2Mask >> a2Shift;
            ps.Mission = (SensorEmitterAppearance.MissionValue)a2Value;

            uint a3Mask = 0x0018;
            byte a3Shift = 3;
            uint a3Value = value & a3Mask >> a3Shift;
            ps.Damage = (SensorEmitterAppearance.DamageValue)a3Value;

            uint a4Mask = 0x0060;
            byte a4Shift = 5;
            uint a4Value = value & a4Mask >> a4Shift;
            ps.Smoke = (SensorEmitterAppearance.SmokeValue)a4Value;

            uint a5Mask = 0x0180;
            byte a5Shift = 7;
            uint a5Value = value & a5Mask >> a5Shift;
            ps.TrailingEffects = (SensorEmitterAppearance.TrailingEffectsValue)a5Value;

            uint a7Mask = 0x1000;
            byte a7Shift = 12;
            uint a7Value = value & a7Mask >> a7Shift;
            ps.Lights = (SensorEmitterAppearance.LightsValue)a7Value;

            uint a9Mask = 0x8000;
            byte a9Shift = 15;
            uint a9Value = value & a9Mask >> a9Shift;
            ps.Flaming = (SensorEmitterAppearance.FlamingValue)a9Value;

            uint a10Mask = 0x10000;
            byte a10Shift = 16;
            uint a10Value = value & a10Mask >> a10Shift;
            ps.Antenna = (SensorEmitterAppearance.AntennaValue)a10Value;

            uint a11Mask = 0x60000;
            byte a11Shift = 17;
            uint a11Value = value & a11Mask >> a11Shift;
            ps.CamouflageType = (SensorEmitterAppearance.CamouflageTypeValue)a11Value;

            uint a12Mask = 0x80000;
            byte a12Shift = 19;
            uint a12Value = value & a12Mask >> a12Shift;
            ps.Concealed = (SensorEmitterAppearance.ConcealedValue)a12Value;

            uint a14Mask = 0x200000;
            byte a14Shift = 21;
            uint a14Value = value & a14Mask >> a14Shift;
            ps.FrozenStatus = (SensorEmitterAppearance.FrozenStatusValue)a14Value;

            uint a15Mask = 0x400000;
            byte a15Shift = 22;
            uint a15Value = value & a15Mask >> a15Shift;
            ps.PowerPlantStatus = (SensorEmitterAppearance.PowerPlantStatusValue)a15Value;

            uint a16Mask = 0x800000;
            byte a16Shift = 23;
            uint a16Value = value & a16Mask >> a16Shift;
            ps.State = (SensorEmitterAppearance.StateValue)a16Value;

            uint a17Mask = 0x1000000;
            byte a17Shift = 24;
            uint a17Value = value & a17Mask >> a17Shift;
            ps.Tent = (SensorEmitterAppearance.TentValue)a17Value;

            uint a19Mask = 0x4000000;
            byte a19Shift = 26;
            uint a19Value = value & a19Mask >> a19Shift;
            ps.BlackoutLights = (SensorEmitterAppearance.BlackoutLightsValue)a19Value;

            uint a21Mask = 0x20000000;
            byte a21Shift = 29;
            uint a21Value = value & a21Mask >> a21Shift;
            ps.InteriorLights = (SensorEmitterAppearance.InteriorLightsValue)a21Value;
