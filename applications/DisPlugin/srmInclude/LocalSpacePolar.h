...
// $Id: LocalSpacePolar.h,v 1.19 2009/12/23 23:42:59 worleym Exp $
...
public:
    ...
    */
    ...
    : Coord2D(srf)
    {
        setComponentValues(radius, angle);
    }
    ...
    void copyTo( Coord2D_LocalSpacePolar &coord ) const
    {
        if (coord._srf != _srf)
            throw Exception( SRM_STATCOD_INVALID_SOURCE_COORDINATE,
                  "copyTo: Coordinate associated with a difference SRF" );
    ...
    */
    void setComponentValues( SRM_Long_Float radius, SRM_Long_Float angle)
    {
        _values[0] = radius;
        _values[1] = angle;
    }
    ...
    */
    bool isCompatibleWith( const Coord2D_LocalSpacePolar &rhs ) const
    {
   ...
   /** The assignment operator
       ...
       @exception This method throws srm::Exception
    */
    Coord2D_LocalSpacePolar &operator= ( const Coord2D_LocalSpacePolar &rhs )
    {
        if((*(SRF_LocalSpacePolar*)(this->_srf)) == (*(SRF_LocalSpacePolar*)(rhs._srf)))
        {
            ...
            throw Exception(SRM_STATCOD_INVALID_TARGET_COORDINATE,
                            "Coord2D_LocalSpacePolar op=: incompatible rhs coordinate");
