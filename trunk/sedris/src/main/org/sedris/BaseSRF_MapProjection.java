...
public abstract class BaseSRF_MapProjection extends BaseSRF_WithEllipsoidalHeight
{
    ...
    // this is a cashing for the ormData used in COM and Point Distortion calculations
    ...
    */
	...
	if ( component == 1 && !(int_interval==SRM_Interval_Type._UNBOUNDED) &&
	     lower<=-Const.PI && lower>Const.PI &&
	     upper<=-Const.PI && upper>Const.PI )
	  throw new SrmException( SrmException._INVALID_INPUT, "setValidGeodeticRegion: component 1 (longitude) out of range (-PI, PI]");  
  
	if ( !(int_interval==SRM_Interval_Type._UNBOUNDED ) && lower == upper )
	  throw new SrmException( SrmException._INVALID_INPUT, "setValidGeodeticRegion: lower and upper bounds are the same");
  
	if ( component == 2 && !(int_interval==SRM_Interval_Type._UNBOUNDED) &&
	     lower<=-Const.PI_DIV_2 && lower>=Const.PI_DIV_2 &&
	     upper<=-Const.PI_DIV_2 && upper>=Const.PI_DIV_2 )
	  throw new SrmException( SrmException._INVALID_INPUT, "setValidGeodeticRegion: component 2 (latitude) out of range (-PI/2, PI/2)");
    ...
    */
	...
	if ( component == 1 && !(int_interval==SRM_Interval_Type._UNBOUNDED) &&
	     lower<=-Const.PI && lower>Const.PI &&
	     upper<=-Const.PI && upper>Const.PI )
	  throw new SrmException( SrmException._INVALID_INPUT, "setExtendedValidGeodeticRegion: component 1 (longitude) out of range (-PI, PI]");  
  
	if ( component == 2 && !(int_interval==SRM_Interval_Type._UNBOUNDED) &&
	     lower<=-Const.PI_DIV_2 && lower>=Const.PI_DIV_2 &&
	     upper<=-Const.PI_DIV_2 && upper>=Const.PI_DIV_2 )
	  throw new SrmException( SrmException._INVALID_INPUT, "setExtendedValidGeodeticRegion: component 2 (latitude) out of range (-PI/2, PI/2)");
  
	if ( !(int_interval==SRM_Interval_Type._UNBOUNDED ) && lower == upper )
	  throw new SrmException( SrmException._INVALID_INPUT, "setExtendedValidGeodeticRegion: lower and upper bounds are the same");
  
	if ( !(int_interval==SRM_Interval_Type._UNBOUNDED ) )
    ...
    */
	...
	double[] tgtCdCoord = interimConv( surf_coord, SRM_SRFT_Code.SRFT_CELESTIODETIC );
