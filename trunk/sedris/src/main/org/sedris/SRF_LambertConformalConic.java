...
public class SRF_LambertConformalConic extends BaseSRF_MapProjection
{
    ...
    */
    ...
    public String toString()
    {
       ...
       retString = retString + "origin_longitude: " + _params.origin_longitude+ "\n";
	...
	if ( _com_n == 123.123 )
		...
		m_phi1 = Math.cos(latitude1) / Math.sqrt(1 - epsilon_squared_sin_squared_lat1);
		m_phi2 = Math.cos(latitude2) / Math.sqrt(1 - epsilon_squared_sin_squared_lat2);

		tan_phi1= Math.tan( Const.PI_DIV_4 - latitude1 * 0.5 ) *
		    Math.pow( (1+epsilon_times_sin_lat1)/(1-epsilon_times_sin_lat1), e_constants.EpsH );

		tan_phi2= Math.tan( Const.PI_DIV_4 - latitude2 * 0.5 ) *
		    Math.pow( (1+epsilon_times_sin_lat2)/(1-epsilon_times_sin_lat2), e_constants.EpsH );
	             
		if ( Const.isEqual( latitude1, latitude2 ) )
		    {
    ...
    {
      double sphi, esphi, tphi, mphi;

      // compute initialization data if not done already
      if ( _lcc_n == 123.123 )
	{
	  double
	    slat1, slat2, e2slat12, e2slat22, eslat1, eslat2,
	    mphi2, tphi2;
	  ...
	  _lcc_mphi1 = Math.cos(latitude1) / Math.sqrt(1 - e2slat12);
	  mphi2 = Math.cos(latitude2) / Math.sqrt(1 - e2slat22);

	  _lcc_tphi1= 
	      Math.tan( Const.PI_DIV_4 - latitude1 * 0.5 ) *
	      Math.pow( (1+eslat1)/(1-eslat1), e_constants.EpsH );
	  tphi2=
	      Math.tan( Const.PI_DIV_4 - latitude2 * 0.5 ) *
	      Math.pow( (1+eslat2)/(1-eslat2), e_constants.EpsH );
	  ...
	  Math.pow( (1+esphi)/(1-esphi), e_constants.EpsH );
      
      mphi = 
	  Math.cos( cd_coord[1] ) /
	  Math.sqrt(1 - esphi*esphi);
