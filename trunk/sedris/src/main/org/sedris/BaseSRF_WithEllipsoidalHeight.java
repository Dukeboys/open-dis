...
public abstract class BaseSRF_WithEllipsoidalHeight extends BaseSRF_3D
{
    ...
    */
	...
	@param hsr_dst in : the RT for the destination SRF Set
    ...
    */
    ...
    {
	if (src_coord == null || orm_dst == null || hsr_dst == null || tgt_srfs == null )
	    throw new SrmException( SrmException._INVALID_INPUT, 
    ...
    */
	...
	return (BaseSRF_3D)CreateSRF.srfSetMember( tgt_srfs, tmpMemberCode, orm_dst, hsr_dst);
    }
	...
	if (tempCcSrf == null)
	    tempCcSrf = new SRF_Celestiocentric( coord1.getSRF().get_orm(), coord1.getSRF().get_rt() );
	...
	double[] tmp3DCoord1 = { coord1.getValues()[0],
				 coord1.getValues()[1],
				 0.0 };

	// converting source coordinate to celestiocentric SRF
	OpManager.instance().computeAsArray( coord1.getSRF(), 
					     tempCcSrf, 
					     tmp3DCoord1,
					     tempCcSrcCoord,
					     null );
	...
	double[] tmp3DCoord2 = { coord2.getValues()[0],
				 coord2.getValues()[1],
				 0.0 };

	// converting target coordinate to celestiocentric SRF
	OpManager.instance().computeAsArray( coord2.getSRF(), 
					     tempCcSrf, 
					     tmp3DCoord2,
					     tempCcTgtCoord,
					     null );
    ...
    */
	...
	double[] tempCdSrcCoord = new double[3];
	double[] tempCdDesCoord = new double[3];
	...
	tempCdSrf = (SRF_Celestiodetic)this._internalSRFs.get("Interim_Cd");
		
	// create a Celestiodetic SRF using the common ORM/RT
	if (tempCdSrf == null)
	    tempCdSrf = new SRF_Celestiodetic( this.get_orm(), this.get_rt() );

		// cache the interim Celestiodetic in the (source) SRF for
		// coordinate 1.  We use the same one for coordinate 2.
	this._internalSRFs.put("Interim_Cd", tempCdSrf);


	// converting source coordinate to celestiodetic SRF
	...
	// converting destination coordinate to celestiodetic SRF
	OpManager.instance().computeAsArray( this, 
					     tempCdSrf, 
					     des_coord.getValues(),
					     tempCdDesCoord,
	...
	OrmData orm_data = new OrmData( this.get_orm() );
	...
	throw new SrmException( SrmException._INVALID_SOURCE_COORDINATE, 
      ...
      if ( dss == SRM_DSS_Code.DSS_UNDEFINED )
	throw new SrmException( SrmException._INVALID_CODE, "calculateVerticalSeparationOffset: Invalid DSS code");
      ...
      if ( dss != SRM_DSS_Code.DSS_EGM96_GEOID ||
           dss != SRM_DSS_Code.DSS_WGS84_ELLIPSOID ||
	   dss != SRM_DSS_Code.DSS_EGM84_GEOID )
	throw new SrmException( SrmException._OPERATION_UNSUPPORTED, "calculateVerticalSeparationOffset: not supported for this DSS");
      else if ( dss == SRM_DSS_Code.DSS_EGM84_GEOID )
	throw new SrmException( SrmException._NOT_IMPLEMENTED, "calculateVerticalSeparationOffset: not implemented for DSS_EGM84_GEOID");
      ...
      tgtCdCoord = interimConv( surf_coord, SRM_SRFT_Code.SRFT_CELESTIODETIC );
	...
	while (Math.abs(deltaomega_old-deltaomega_new) > 1.e-12)
	  {
	    ...
	    // the following equation uses "hypot" which us not current supported in Java 2.1.4
	    // We will explicitely implement it as sqrt(x*x + y*y).
// 	    sigma = Math.atan2( Math.hypot(cos_u2*sin_omega, cos_u1*sin_u2-sin_u1*cos_u2*cos_omega),
// 				sin_u1*sin_u2+cos_u1*cos_u2*cos_omega );
	    ...
	    sinun2 = 1.-cosun*cosun;   
	    if (sinun2 == 0.)
	      sigmam = Math.acos(cos_sigma-2.);
	    else
	      sigmam = Math.acos(cos_sigma-2.*sin_u1*sin_u2/sinun2+1.e-12);   
              //                                                   ^temporary fix to avoid -1.#IND
	    cos_sigmam = Math.cos(sigmam);
	    cos_2sigmam = Math.cos(2*sigmam);

	    v = 0.25*f*sinun2;
	    K3 = v*(1+f+f*f-v*(3+7*f-13*v));
	    deltaomega_new = (1-K3)*f*cosun*(sigma+K3*sin_sigma*(cos_sigmam+K3*cos_sigma*cos_2sigmam));
	  }
	...
	deltasigma = K2*sin_sigma*(cos_sigmam+
				   K2*(cos_sigma*cos_2sigmam+
				       K2*(1+2*Math.cos(2*sigma))*Math.cos(3*sigmam)/6)/4);

	geodesic_distance = K1*(1-f)*a*(sigma-deltasigma);
 ...
 //         System.out.println( "Table size=> " + total_token );
  ...
  (
   SRM_DSS_Code  dss,
   double[]      coord_org_wgs84_cd ) throws SrmException
  {	
    ...
    if ( dss == SRM_DSS_Code.DSS_WGS84_ELLIPSOID )
	return 0;
