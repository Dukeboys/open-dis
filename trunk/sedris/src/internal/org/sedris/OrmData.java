    ...
    public OrmData(SRM_ORM_Code orm)
    {
        ...
        if (orm.toInt() >= 0)
        {
            if (rdCode != SRM_RD_Code.RDCOD_UNSPECIFIED)
            {
                ...
                if (tmpORMT == SRM_ORMT_Code.ORMTCOD_SPHERE ||
                    tmpORMT == SRM_ORMT_Code.ORMTCOD_SPHERE_ORIGIN)
                {
                    A      = rdSet._R;
                    F_inv  = Double.NaN;
                    F      = 0.0;
                ...
                else if (tmpORMT == SRM_ORMT_Code.ORMTCOD_OBLATE_ELLIPSOID ||
                         tmpORMT == SRM_ORMT_Code.ORMTCOD_OBLATE_ELLIPSOID_ORIGIN ||
                         tmpORMT == SRM_ORMT_Code.ORMTCOD_TRI_AXIAL_ELLIPSOID)
                {
                    A     = rdSet._A;
                    F_inv = rdSet._inv_F;
                    F     = 1.0/F_inv;
        ...
        {
            if (orm == SRM_ORM_Code.ORM_EXPERIMENTAL_NGA_MAX)
            {
                A     = 6400000.0;
                F_inv = 150.0;
                F     = 1.0/F_inv;
            }
            else if (orm == SRM_ORM_Code.ORM_EXPERIMENTAL_NGA_SPHERE)
            {
