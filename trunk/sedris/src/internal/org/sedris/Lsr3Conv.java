...
class Lsr3Conv extends LsrConv
{
	...
	super (SRM_SRFT_Code.SRFT_LOCAL_SPACE_RECTANGULAR_3D,
	       new SRM_SRFT_Code[] {SRM_SRFT_Code.SRFT_LOCAL_SPACE_RECTANGULAR_3D,
 ...
 */
	...
	if ( destSrfType == SRM_SRFT_Code.SRFT_LOCAL_SPACE_RECTANGULAR_3D )
	    // No restrictions
	...
	if ( lsr_3d_SRF_params.forward_direction == SRM_Axis_Direction.POS_PRIMARY_AXIS ) {
	    Const.copyArray( e1, primary_axis ); /*Implicitly multiplied by 1 for positive primary axis*/
	}	
	else if (lsr_3d_SRF_params.forward_direction == SRM_Axis_Direction.POS_SECONDARY_AXIS) {
	    Const.copyArray(e2, primary_axis); /*Implicitly multiplied by 1 for positive secondary axis*/
	}	
	else if (lsr_3d_SRF_params.forward_direction == SRM_Axis_Direction.POS_TERTIARY_AXIS) {
	    Const.copyArray(e3, primary_axis); /*Implicitly multiplied by 1 for positive tertiary axis*/
	}	
	...
	if(lsr_3d_SRF_params.up_direction == SRM_Axis_Direction.POS_PRIMARY_AXIS) {
	    Const.copyArray(e1, secondary_axis); /*Implicitly multiplied by 1 for positive primary axis*/
	}	
	else if(lsr_3d_SRF_params.up_direction == SRM_Axis_Direction.POS_SECONDARY_AXIS) {
	    Const.copyArray(e2, secondary_axis); /*Implicitly multiplied by 1 for positive secondary axis*/
	}
	else if(lsr_3d_SRF_params.up_direction == SRM_Axis_Direction.POS_TERTIARY_AXIS) {
	    Const.copyArray(e3, secondary_axis); /*Implicitly multiplied by 1 for positive tertiary axis*/
	}	
	...
	else
	{
	    /*Following ISO 18026 and a personal communication
	      ...
	      correct nomenclature for the unit vectors in LSR is primary_axis,secondary_axis, and t.
