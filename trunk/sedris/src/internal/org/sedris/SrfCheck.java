...
class SrfCheck
{
    ...
    ) throws SrmException
    {
        if (orm.toInt() > 0)
        {
            if (OrmDataSet.getElem(orm)._orm_template !=
                SRM_ORMT_Code.ORMTCOD_SPHERE &&
                OrmDataSet.getElem(orm)._orm_template !=
                SRM_ORMT_Code.ORMTCOD_OBLATE_ELLIPSOID &&
                OrmDataSet.getElem(orm)._orm_template !=
                SRM_ORMT_Code.ORMTCOD_SPHERE_ORIGIN &&
                OrmDataSet.getElem(orm)._orm_template !=
                SRM_ORMT_Code.ORMTCOD_OBLATE_ELLIPSOID_ORIGIN &&
                OrmDataSet.getElem(orm)._orm_template !=
                SRM_ORMT_Code.ORMTCOD_PROLATE_ELLIPSOID &&
                OrmDataSet.getElem(orm)._orm_template !=
                SRM_ORMT_Code.ORMTCOD_TRI_AXIAL_ELLIPSOID &&
                OrmDataSet.getElem(orm)._orm_template !=
                SRM_ORMT_Code.ORMTCOD_BI_AXIS_ORIGIN_3D &&
                OrmDataSet.getElem(orm)._orm_template !=
                SRM_ORMT_Code.ORMTCOD_TRI_PLANE)
            {
    ...
    ) throws SrmException
    {
        ...
        if ((params.polar_aspect != SRM_Polar_Aspect.NORTH) &&
            (params.polar_aspect != SRM_Polar_Aspect.SOUTH))
        {
            throw new SrmException(SrmException._INVALID_SOURCE_SRF,
                                   new String("SRF_PolarStereographic: " +
