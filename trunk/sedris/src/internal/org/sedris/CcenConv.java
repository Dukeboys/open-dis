...
class CcenConv extends Conversions
{
	...
	if ( destSrfType == SRM_SRFT_Code.SRFT_LOCOCENTRIC_EUCLIDEAN_3D )
	    {
	      if (_toLceConst == null)
		_toLceConst = new ToLceConst(((SRF_LococentricEuclidean3D)destSrf).getSRFParameters());
		...
		// apply the false origin offset after the conversion if LTSE_3D is
		// the destination SRF.  Otherwise do nothing because LTSE_3D is just
		// an intermediate SRF
 ...
 */
	...
	public ToCdetConst(OrmData e_constants)
	{
	    ...
	    if ( e_constants.Eps > Const.EPSILON_SQ )
		{
		    ...
		    ak1 = e_constants.C * e_constants.Eps2 / ( 1 - e_constants.Eps2);
			...
			d1 = Const.square(e_constants.C + hmx) * g3 - Const.square(e_constants.C + hmn) * g4;
			d1 = d1 / ( Const.square(e_constants.C + hmx) - Const.square(e_constants.C + hmn) );
			...
			d6 = 1 / Const.square(e_constants.A + hmx) - 1 / Const.square(e_constants.A + hmn);
     
			d6 = d6 * d4 / (g2 - g1);

			sm = Const.ONE_DIV_ROOT_2;  /*sin(SRM_PI_DIV_4)=1/sqrt(2)*/

			rnm = e_constants.A / Math.sqrt(1 - e_constants.Eps2 * Const.square(sm) );
	...
	double alpha, arg2, cf, cl, lococentre, r1, r2, ro, roe, rr, secondary_axis, top,
	    top2, v, w, w2, xarg, z2, zo;
  
	double lowerBound, upperBound,ga,bot;
	...
	w2=Const.square(source_generic_coord[0]) + Const.square(source_generic_coord[1]);
	...
	if( !done_by_special_case || region == REGION_SPHERICAL )
		...
		if (region != REGION_SPHERICAL)
			...
			if(( lowerBound >= _toCdetConst.v[REGION_2]) &&  (upperBound <= _toCdetConst.v[REGION_3]))
			    {
				region = REGION_2;
			    }
			else
				...
				if((lowerBound >= _toCdetConst.v[REGION_3]) && (upperBound <= _toCdetConst.v[REGION_4]))
				    {
					region = REGION_3;
				    }
				else
				...
				/* region 1 -30 to 0 kilometers */
					...
					if((lowerBound >= _toCdetConst.v[REGION_1]) && (upperBound <= _toCdetConst.v[REGION_2]))
					    {
						region = REGION_1;
					    }
					else
						...
						if((lowerBound >=  _toCdetConst.v[REGION_4] ) && ( upperBound <=  _toCdetConst.v[REGION_5]))
						    {
							region = REGION_4;
						    }
						else
		...
		*/
		...
		switch( region )
			...
			{	  	  
				...
				dest_elv_ptr = lococentre - Rn;
			    else
				dest_elv_ptr = lococentre / ga + _toCdetConst.aeps21 * Rn * e_constants.A_inv;
...

			{
				...
				if(s12 <= 0.5)
				    {
					dest_elv_ptr = w * lococentre / bot - Rn;
				    }
				else
				    {
					dest_elv_ptr = source_generic_coord[2] * lococentre / top + 
		    ...
		    case REGION_5:
			{
			    ...
			    secondary_axis = Math.pow(s1,(.333333333333333333333333333333));
			    {
				double temp = secondary_axis / (Const.square(secondary_axis) + 1.0 + secondary_axis);
				p = alpha * .333333333333333333333333333 * Const.square(temp) ;
			    }
			    xarg = 1.0 + (_toCdetConst.TwoCEE * p);
			    lococentre = Math.sqrt(xarg);
	    
			    {
				double q_inv = 1.0/lococentre;
				double one_plus_q_inv = 1.0/(1.0+lococentre);
 ...
 *----------------------------------------------------------------------------
 */
    protected void toSphere (BaseSRF           srcSrf,
			     BaseSRF           destSrf,
			     double[]          source_generic_coord,
			     double[]          dest_generic_coord) throws SrmException
    {  
