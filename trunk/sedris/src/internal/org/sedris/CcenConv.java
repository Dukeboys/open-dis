...
class CcenConv extends Conversions
{
    ...
    ) throws SrmException
    {
        ...
        else if (destSrfType == SRM_SRFT_Code.SRFT_LOCOCENTRIC_EUCLIDEAN_3D)
        {
             if (_toLceConst == null)
        ...
        else if (destSrfType == SRM_SRFT_Code.SRFT_LOCAL_TANGENT_SPACE_EUCLIDEAN)
        {
             ...
             retValid = CoordCheck.forNaN_3D(dest);
              ...
              || (destSrfType == SRM_SRFT_Code.SRFT_EQUATORIAL_INERTIAL)
              || (destSrfType == SRM_SRFT_Code.SRFT_SOLAR_ECLIPTIC)
              || (destSrfType == SRM_SRFT_Code.SRFT_SOLAR_EQUATORIAL)
              || (destSrfType == SRM_SRFT_Code.SRFT_SOLAR_MAGNETIC_DIPOLE)
              || (destSrfType == SRM_SRFT_Code.SRFT_SOLAR_MAGNETIC_ECLIPTIC)
              || (destSrfType == SRM_SRFT_Code.SRFT_HELIOSPHERIC_ARIES_ECLIPTIC)
              || (destSrfType == SRM_SRFT_Code.SRFT_HELIOSPHERIC_EARTH_ECLIPTIC)
              || (destSrfType == SRM_SRFT_Code.SRFT_HELIOSPHERIC_EARTH_EQUATORIAL))
        {
