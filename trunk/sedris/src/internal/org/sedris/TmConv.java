...
class TmConv extends Conversions
{
	...
	super(SRM_SRFT_Code.SRFT_TRANSVERSE_MERCATOR,
	      new SRM_SRFT_Code[] {SRM_SRFT_Code.SRFT_CELESTIODETIC, 
 ...
 */
    ...
    {	
	/*This software abomination computes spherical TM_AUGMENTED_3D coordinates for GtrsDataSet.getNaturalCell( coord )
	  a bounding box for TM_AUGMENTED_3D shaped like this to ensure that horrible
	...
	public TmValidConst( OrmData e_constants, SRF_TM_Params params ) throws SrmException
	{
	    ...
	    double left_lon_val = Const.getLambdaStar( params.origin_longitude,
						       -13.0 * Const.RADIANS_PER_DEGREE );

	    double[] uln_cd = new double[]{left_lon_val, 89.991*Const.RADIANS_PER_DEGREE, 0.0};
	    double[] uls_cd = new double[]{left_lon_val, 89.990*Const.RADIANS_PER_DEGREE, 0.0};
 ...
 */
	...
	public ToCdetConst(OrmData e_constants, SRF_TM_Params tmParams)
	{
	    ...
	    if (e_constants.Eps!=0.0) /* compute ellipsoidal values */
	    {
	      ...
	      smz = Const.exactArcLength(e_constants.A,e_constants.C,e_constants.Eps2, latitude_origin);
	...
	/*mu is the parameterization variable of footpoint latitude which is
	  ...
	  if (e_constants.Eps!=0.0) /* compute ellipsoidal values */
	    {
	      /*This ellipsoidal tm_cd routine is a new formulation 
	  ...
	  double
	    ...
	    double Rn1_inv=0.0,         /*1 over the radius of curvature in the prime vertical*/	
		       phi=0.0,                          /*latitude*/
		       lambda=0.0;                       /*longitude*/	
	    ...
	    Rn1_inv = Const.computeRnInv(sin_phi1_squared, e_constants);
	      ...
	      /*    
		    Mu is the variable that footpoint_latitude is parameterized in.  The following definition is
		    correct but the above form using precomputed constants that is actually used is more accurate:
		    	    
		    mu=src_coord[1]/
		    (e_constants->A*
		    (1-SQUARE(e_constants->Eps)*((1.0/4.0)+SQUARE(e_constants->Eps)*
		    ((3.0/64.0)+(5.0/256.0)*SQUARE(e_constants->Eps)))));
		    
		    
		    Rn1, the radius of curvature in the prime vertical is defined thus but 1 over it
		    is actually computed to save a sqrt:
		    Rn1=e_constants->A/sqrt(omessp);
		    
		    Rm1, the radius of curvature in the meridian is defined as follows but not needed:
	      ...
	      */
	      ...
	      /*
		phi=phi1-(tan_phi1*omessp/omes)*SQUARE(zeta)*((1.0/2.0)+SQUARE(zeta)*((-b4/24.0)+(b6/720.0)*SQUARE(zeta)));
		lambda=_toCdetConst.longitude_origin+zeta*sec_phi1*(1+SQUARE(zeta)*((-b3/6.0)+b5/120.0*SQUARE(zeta)));
		
		These expressions were derived from a draft of the writeup.  Both are known good with the following
		definitions of the b'secondary_axis in this function.  The actually implemented code removes a number of
		...
		if(lon_range< ZERO_POINT_THREE_DEGREES )
		...
		else
		  {
		    double b4=5.0+Const.triple(tan_phi1_squared)+
		      eta_sub1_squared*(1.0-4.0*eta_sub1_squared- 9.0 *tan_phi1_squared);
		...
		/* Longitude ranges 0.75 2.7 >2.7  Note, lat test is used because the longitude formula
		   here depends on the latitude we're at*/
		{
		  ...
		  if(lon_range< ZERO_POINT_SEVEN_FIVE_DEGREES )
		    {
		      lambda=_toCdetConst.longitude_origin+zeta*(1-b3*con_lon6*zeta_squared)*sec_phi1;
		  }
		  else
		    {
	    ...
	    {
	      /*Compute spherical values here*/

	      /*This spherical derivation is an adaptation of the
		original 3.x and earlier SEDRIS code'secondary_axis spherical TM_AUGMENTED_3D
		routine.  It has been updated for notation and it
		has also been modified to move it'secondary_axis initializations to 
		the sperical section to minimize the possiblity of 
		misinitialized variables.
	      */

	      double
		phi1=(_toCdetConst.smz + source_generic_coordinate[1]/_toCdetConst.CScale)*_toCdetConst.conap_inv,
		sin_phi1=Math.sin(phi1),
		cos_phi1=Math.cos(phi1),
		sec_phi1=1.0/cos_phi1,
		tan_phi1=sin_phi1*sec_phi1,
		tan_phi1_squared=Const.square(tan_phi1),
		d1= source_y*e_constants.A_inv,	
		d2 = d1 * d1,
		b3= 1.0+tan_phi1_squared,
		b4= 5.0+3.0 * tan_phi1_squared,
		b5= 5.0 + tan_phi1_squared * (tan_phi1_squared * 24.0 + 28.0),
		b6= 61.0 + tan_phi1_squared * (tan_phi1_squared * 45.0 + 90.0);

	      	// lat
	    dest_generic_coordinate[1] = phi1 - tan_phi1 *0.25  * (d2 * (2.0 +
							         d2 * ((-.166666666666666) * b4 +
								     d2 * .0055555555555555 * b6)));

	    // lon
	    dest_generic_coordinate[0] = _toCdetConst.longitude_origin +
									 d1 * (1.0 + d2 * (-.16666666666666 * b3 +
				                     d2 * .00833333333333333 * b5))*sec_phi1;
