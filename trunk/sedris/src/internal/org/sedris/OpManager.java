...
public class OpManager
{
	...
	if ( srcSrf.get_orm() == tgtSrf.get_orm()  &&
	     srcSrf.get_rt() == tgtSrf.get_rt() &&
	     hst == null ) { // this case is the case for user supplied xformation params.
 ...
 //			       tgtSrf.getSRFTemplateCode());
		    ...
		    Vector path = Bfs.instance().GetConversionPath(srcSrf.getSRFTemplateCode(), 
								   tgtSrf.getSRFTemplateCode());
 ...
 // 				System.out.println("Getting function for=> " + path.elementAt(i).toString());
		...
		// most cases
			...
			path =	Bfs.instance().GetConversionPath(srcSrf.getSRFTemplateCode(), 
								 tgtSrf.getSRFTemplateCode());
...
//  			    System.out.println("Getting function for=> " + path.elementAt(i).toString());
	...
	// the ORMs and HSRs are different
	    ...
	    Vector path1 = Bfs.instance().GetConversionPath(srcSrf.getSRFTemplateCode(), 
	    ...
	    Vector path2 = Bfs.instance().GetConversionPath(SRM_SRFT_Code.SRFT_CELESTIOCENTRIC, 
							    tgtSrf.getSRFTemplateCode());
	    if (path2 == null) {
		throw new SrmException( SrmException._OPERATION_UNSUPPORTED, 
					new String("No conversion possible") );
...
// 			System.out.println("Getting function for=> " + path2.elementAt(i).toString());
	...
	// reject the conversion if one the RT is UNDEFINED and the other is not.
	if ( (srcSrf.get_orm() != tgtSrf.get_orm() ) && hst == null &&
	     (srcSrf.get_rt() == SRM_RT_Code.RT_UNDEFINED || 
	      tgtSrf.get_rt() == SRM_RT_Code.RT_UNDEFINED) )
	    throw new SrmException( SrmException._INVALID_SOURCE_SRF, 
				    new String("changeCoordinateXX: SRFs with different ORMs and RT undefined " + 
	...
	int srf_iterations;
	if (srcSrf.get_rt() == tgtSrf.get_rt() &&  // XXX Need to verify if cover all cases.
		hst == null )
