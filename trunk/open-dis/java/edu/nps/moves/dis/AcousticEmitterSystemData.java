 ...
 * This requires manual cleanup; the beam data should not be attached to each emitter system.
 *
...
public class AcousticEmitterSystemData extends Object implements Serializable {
    ...
    public boolean equals(AcousticEmitterSystemData rhs) {
        ...
        for (int idx = 0; idx < beamRecords.size(); idx++) {
            AcousticBeamData x = beamRecords.get(idx);
            if (!(x.equals(rhs.beamRecords.get(idx)))) {
                ivarsEqual = false;
            }
        }
