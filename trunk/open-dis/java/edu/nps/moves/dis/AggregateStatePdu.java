...
public class AggregateStatePdu extends EntityManagementFamilyPdu implements Serializable {
     ...
     * The method is simply here for java bean completeness.
     * @param pNumberOfSilentAggregateTypes
     */
     ...
     * The method is simply here for java bean completeness.
     * @param pNumberOfSilentEntityTypes
     */
    ...
    public boolean equals(AggregateStatePdu rhs) {
        ...
        for (int idx = 0; idx < aggregateIDList.size(); idx++) {
            AggregateID x = aggregateIDList.get(idx);
            if (!(x.equals(rhs.aggregateIDList.get(idx)))) {
                ivarsEqual = false;
            }
        }

        for (int idx = 0; idx < entityIDList.size(); idx++) {
            EntityID x = entityIDList.get(idx);
            if (!(x.equals(rhs.entityIDList.get(idx)))) {
                ivarsEqual = false;
            }
        }
        ...
        for (int idx = 0; idx < silentAggregateSystemList.size(); idx++) {
            EntityType x = silentAggregateSystemList.get(idx);
            if (!(x.equals(rhs.silentAggregateSystemList.get(idx)))) {
                ivarsEqual = false;
            }
        }

        for (int idx = 0; idx < silentEntitySystemList.size(); idx++) {
            EntityType x = silentEntitySystemList.get(idx);
            if (!(x.equals(rhs.silentEntitySystemList.get(idx)))) {
                ivarsEqual = false;
            }
        }
        ...
        for (int idx = 0; idx < variableDatumList.size(); idx++) {
            VariableDatum x = variableDatumList.get(idx);
            if (!(x.equals(rhs.variableDatumList.get(idx)))) {
                ivarsEqual = false;
            }
        }
